...
func (s *Service) CreatePaymentUrl(ipAddr string, userId uuid.UUID, paymentId int64, data *dto.VNPCreatePaymentUrl) (string, error) {
	// Get payment by id
...
func (s *Service) Return(query map[string]string) error {
...
func (s *Service) Ipn(query map[string]string) IpnReturn {
	secureHash := query["vnp_SecureHash"]
...
func (s *Service) Querydr(ipAddr string, d *dto.VNPQuerydr) (*http.Response, error) {
	tz, err := time.LoadLocation("Asia/Ho_Chi_Minh")
	if err != nil {
		return nil, err
	}
...
func (s *Service) Refund(ipAddr string, d *dto.VNPRefund) (*http.Response, error) {
	tz, err := time.LoadLocation("Asia/Ho_Chi_Minh")
	if err != nil {
		return nil, err
	}
